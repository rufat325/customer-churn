...
|   +-- security.py            rate limiting, optional auth
...
## 17. Security and public deployment

The dashboard is deployed publicly, on purpose. Full detail in
[`DEPLOYMENT.md`](DEPLOYMENT.md).

**Why that is defensible here:** every customer is synthetic, generated by
`src/generate_data.py`. There is no real person in the dataset, so a public
dashboard exposes nobody.

**Why it would not be with real data:** `GET /customers` pages through ids,
country, spend, order history and churn scores. Unauthenticated, against real
customers, that is a data breach rather than a demo. The password switch exists
for exactly that case.

What is in place:

- **Per-IP rate limiting**, 120 requests/minute by default, sliding window.
  Caps compute abuse and makes model-extraction querying slow and conspicuous.
  `/health` is exempt so the container health check is never throttled.
- **HTTPS**, terminated by Caddy with an automatically renewed Let's Encrypt
  certificate. No domain needed — a wildcard-DNS hostname such as
  `1-2-3-4.nip.io` resolves to the instance and is certificate-eligible.
- **The API is not published on the host.** Caddy reaches it over the private
  compose network, so nothing can bypass TLS or forge the `X-Forwarded-For`
  header the rate limiter trusts.
- **Read-only.** No endpoint writes anything.
- **Non-root container**, uid 10001.
- **Optional auth, off by default** — `CHURN_PASSWORD` puts HTTP Basic over
  everything, `CHURN_API_KEY` guards the scoring endpoints while leaving the
  dashboard browsable. Both compare in constant time. Off is the right default
  for a demo whose purpose is being opened by strangers.
- **SSH restricted** to a single IP; `.pem` files git-ignored. Port 8000 is
  closed at the security group — an earlier deployment left it open to the
  world, which is what made the unauthenticated API reachable from any device.

Still missing for anything real: a shared rate-limit store (the current one is
per-process, so replicas multiply the allowance), audit logging, secrets
management, and an authenticated admin path separate from the public read-only
view.
